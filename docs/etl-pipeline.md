...
### 2.2  CDC Tick Surveillance (`fetch_cdc_ticks.sh`)
...
### 2.3  FARS Deer Collisions (`fetch_fars.sh`)
...
### 2.4  NSSP ED Tick‑Bite Visits (`fetch_ed.sh`)
...
### 2.5  NLCD Land‑Cover & Edge Metrics
...
### 2.6  Census County Reference (`tickbiterisk etl county-reference`)
...
### 2.7  Census Population (`tickbiterisk etl census-population`)
...
### 2.8  NOAA Weather Features (`tickbiterisk etl noaa-weather-features`)
...
### 2.9  Maryland DNR Deer Harvest (`tickbiterisk etl deer-harvest`)
...
### 2.12  Census Building Permits (`tickbiterisk etl building-permits`)
...
### 2.13  Contact Pressure Features (`tickbiterisk etl contact-pressure`)
...
### 2.14  Mast/Acorn Features (`tickbiterisk etl mast-acorn`)
