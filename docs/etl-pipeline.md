...
### 2.3  CDC Tick Surveillance (`fetch_cdc_ticks.sh`)
...
### 2.4  FARS Deer Collisions (`fetch_fars.sh`)
...
### 2.5  NSSP ED Tick‑Bite Visits (`fetch_ed.sh`)
...
### 2.6  NLCD Land‑Cover & Edge Metrics
...
### 2.7  Census County Reference (`tickbiterisk etl county-reference`)
...
### 2.8  Census Population (`tickbiterisk etl census-population`)
...
### 2.9  NOAA Weather Features (`tickbiterisk etl noaa-weather-features`)
...
### 2.10  Maryland DNR Deer Harvest (`tickbiterisk etl deer-harvest`)
...
### 2.13  Census Building Permits (`tickbiterisk etl building-permits`)
...
### 2.14  Contact Pressure Features (`tickbiterisk etl contact-pressure`)
...
### 2.15  Mast/Acorn Features (`tickbiterisk etl mast-acorn`)
