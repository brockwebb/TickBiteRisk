...
| `noaa_cdo_ghcnd_daily` | NOAA CDO Daily Summaries / GHCND | `https://www.ncei.noaa.gov/cdo-web/webservices/v2` | API JSON | Station observations mapped to Maryland counties | 1992-current planned | Primary observed historical raw daily weather input for `noaa_ghcnd_daily_observations`; aggregate to weekly/monthly/seasonal modeling features | candidate, etl_supported, full_acquisition_pending | Public federal data; derived aggregates may be published with citation | Requires `NOAA_TOKEN` from environment only; ETL discovers county stations, uses calendar-year daily windows plus paginated CDO reads, and can orchestrate all Maryland counties or a subset; daily is not the modeling granularity |
...
- NOAA CDO/GHCND full Maryland daily weather acquisition run; the orchestration command exists, and daily weather is raw input to aggregate weekly, monthly, and seasonal model features.
