...
| `noaa_cdo_ghcnd_daily` | NOAA CDO Daily Summaries / GHCND | `https://www.ncei.noaa.gov/cdo-web/webservices/v2` | API JSON | Station observations mapped to Maryland counties | 1992-current planned | Primary observed historical raw daily weather input for `noaa_ghcnd_daily_observations`; aggregate to weekly/monthly/seasonal modeling features | candidate, station_audited, etl_supported, fallback_needed, full_acquisition_pending | Public federal data; derived aggregates may be published with citation | Requires `NOAA_TOKEN` from environment only; strict 1992-current internal-station audit found 11/24 jurisdictions ready and 13/24 needing fallback station logic; daily is not the modeling granularity |
...
- NOAA CDO/GHCND fallback station strategy for counties without strict 1992-current internal station coverage, then the full Maryland daily weather acquisition run.
