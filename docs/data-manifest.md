...
| `noaa_psl_mei_v2` | NOAA PSL Multivariate ENSO Index version 2 | `https://psl.noaa.gov/enso/mei/` and `https://psl.noaa.gov/data/correlation/meiv2.csv` | CSV/time series | Global monthly climate index | 1979-present for MEI.v2 table | ENSO ocean-atmosphere climate context candidate | candidate, needs_acquisition, needs_etl, optional | Public federal data; cite NOAA PSL | Companion to ONI/RONI if ENSO is tested. MEI.v2 combines multiple ocean-atmosphere variables, so it belongs in a separate lagged feature lane rather than silently replacing local Maryland weather or drought features. |
...
| `model_features_county_year` | Maryland county-year model feature matrix | `build/etl/model/model_features_county_year.csv` | CSV | County-year | 1992-2023 | Auditable joined training panel | etl_materialized | Derived from public sources; publish with source citations and quality flags | Built by `tickbiterisk etl model-features`; refreshed 2026-05-28 run wrote 676 rows across 24 Maryland jurisdictions. Required joins: Lyme outcomes, population, calendar-apportioned weekly NOAA weather. Optional joins now include contact pressure with construction lags, prior-season deer harvest, prior-year mast/acorn, same-year and prior-year USDM drought, EnviroAtlas static habitat, tick status, and complete prior-year NOAA CPC ONI climate context. Prior-year drought is populated for 551 rows and ONI is populated for all 676 rows. Mast/acorn and prior-year drought values join observation year N to model year N+1; ONI is a global prior-year signal and remains not Maryland-specific. |
| `model_design_matrix_county_year` | Numeric Maryland county-year model design matrix | `build/etl/model/model_design_matrix_county_year.csv` and `build/etl/model/model_design_matrix_schema.json` | CSV + JSON schema | County-year | 1993-2023 rows with prior county history | Model-consumable feature matrix for stdlib baselines, ridge profiles, empirical-Bayes shrinkage, and future model lanes | etl_materialized | Derived from public sources; publish schema/caveats with model artifacts | Built by `tickbiterisk etl model-design-matrix`; 2026-05-28 live run with 5-year lookback and county adjacency wrote 652 rows. Keeps id/target/passthrough columns separate from numeric `feature_*` columns, adds lagged incidence features, one-hot tick status features, optional-source missingness indicators, spatial prior-year neighbor incidence features, and numeric drought/habitat/construction-lag/ONI predictors. Forecast spatial selectors include prior-year neighbor incidence; forecast ecology selectors include prior-year drought and prior-year ONI; same-year drought remains retrospective-only. |
| `ecological_pressure_index` | Composite ecological pressure feature candidate | Derived feature candidate | CSV/derived columns | County-year | Future design matrix lane | Combinatorial ecology/contact/climate/spatial pressure feature candidate | derived_feature_candidate, needs_design, needs_backtest, not_public_default | Derived from source-attributed public inputs; preserve component provenance | Proposed lane would combine timing-safe components such as prior-year mast/acorn, prior-season deer harvest, prior-year drought, static habitat/forest composition, construction/contact lags, prior-year neighbor incidence, and optional ENSO/Open-Meteo/FIA inputs. Keep component values and flags visible; do not hide weak or missing source quality inside a single score. |
| `model_comparison` | Rolling-origin annual model comparison | `build/etl/model-comparison/model_comparison_runs.csv`, `model_comparison_predictions.csv`, `model_comparison_metrics.csv`, `model_comparison_summary.csv` | CSV | County-year prediction/evaluation | Held-out years 2007-2023 | Model selection benchmark over normalized design matrix | etl_materialized | Derived evaluation artifact; publish with model caveats | Built by `tickbiterisk etl model-compare`; 2026-05-28 run wrote 1 run row, 3,264 prediction rows, 144 metric rows, and 8 summary rows after adding complete prior-year ONI to the forecast ecology profile. Summary MAE per 100k: `linear_blend_baseline` 18.240783, `prior_year_incidence` 18.32327, `ridge_forecast_safe` 19.094881, `ridge_forecast_spatial` 19.222024, `ridge_forecast_ecology` 22.28845, `ridge_lag_weather_ecology` 22.652277, `empirical_bayes_shrinkage` 22.859609, `trailing_mean_incidence` 22.903909. The spatial, ecology, and ONI-augmented ecology features did not improve held-out MAE, so the selected public branch remains `linear_blend_baseline`. |
...
- NOAA CPC ONI is materialized into `build/etl/enso/noaa_cpc_oni_seasons.csv` and `build/etl/enso/noaa_cpc_oni_model_year_features.csv`; it is not joined into the public-default model.
- NOAA CPC RONI, NOAA PSL MEI.v2, citizen-science tick observations, GBIF tick occurrences, and the composite ecological-pressure index are catalogued as candidates only. They are not ETL materialized and are not public-default model inputs.
...
- RONI/MEI extraction and the model-feature join/backtest contract if a broad climate-regime feature is tested beyond the materialized ONI lane.
- Citizen-science tick observation extraction and effort-normalization contract if iNaturalist or GBIF is tested.
- Composite ecological-pressure feature contract and backtest lane if combined interaction features are tested.
...
Still pending for later feature matrix versions: FIA/NLCD forest-composition summaries if justified, 2024+ population denominators, and ZIP/ZCTA lookup.
