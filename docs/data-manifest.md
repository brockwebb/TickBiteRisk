...
| `noaa_cdo_ghcnd_daily` | NOAA CDO Daily Summaries / GHCND | `https://www.ncei.noaa.gov/cdo-web/webservices/v2` | API JSON | Station observations mapped to Maryland counties | 1992-current planned | Primary observed historical raw daily weather input for `noaa_ghcnd_daily_observations`; aggregate to weekly/monthly/seasonal modeling features | candidate, etl_supported, backfill_pending | Public federal data; derived aggregates may be published with citation | Requires `NOAA_TOKEN` from environment only; ETL uses calendar-year daily windows plus paginated CDO reads; daily is not the modeling granularity |
...
- NOAA CDO/GHCND daily weather backfill for Maryland jurisdictions; daily weather is raw input to aggregate weekly, monthly, and seasonal model features.
