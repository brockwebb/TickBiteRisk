...
| `epa_enviroatlas` | EPA EnviroAtlas national data downloads | `https://www.epa.gov/enviroatlas/data-download` | HTML/catalog now; county CSV batch later | National county tables where available | Current downloadable national tables | Static habitat/environment covariates | source_manifested, acquisition_candidate | Public federal data | Official no-raster shortcut candidate for county habitat features; defer the 150 MB batch download and table selection until the mast/acorn modeling refresh is reviewed |
...
The v0 comparison uses rolling-origin validation from the numeric design matrix and compares seven stdlib model profiles: prior-year incidence, trailing mean incidence, linear blend baseline, empirical-Bayes shrinkage, forecast-safe ridge over lag/history features, forecast-safe ridge with prior-season deer and prior-year mast ecology, and retrospective ridge with observed-year weather/contact plus lagged ecology. Current cumulative tick-status features and surveillance-era flags are excluded from the ridge profiles. Forecast-safe profiles do not use same-year observed weather or same-year construction/contact pressure; observed-year weather/contact features are treated as retrospective reconstruction, not prospective forecast inputs. Mast/acorn observations are joined only as observation year N to model year N+1. The 2026-05-26 live run ranked the linear blend baseline first by MAE.
...
Still pending for later feature matrix versions: EnviroAtlas/FIA habitat summaries, 2024+ population denominators, and ZIP/ZCTA lookup.
