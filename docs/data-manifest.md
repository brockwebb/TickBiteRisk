...
| `county_week_seasonal_risk_baseline` | Relative county-week Lyme seasonal risk baseline | `build/etl/county-week-risk/county_week_seasonal_risk_baseline.csv` and `build/etl/county-week-risk/risk_score_scale.csv` | CSV | Maryland county-week | Held-out prediction years from baseline backtests | Product-shaped relative risk score baseline | etl_materialized, runtime_supported | Derived artifact; publish with source citations, model version, and caveats | Built by `tickbiterisk etl county-week-risk`; 2026-05-25 live smoke with `linear_blend_baseline` wrote 21,571 county-week rows and 1 scale row. Combines an annual backtest prediction branch with the selected weekly seasonality source, default `cdc_seasonality_week_2023`, then maps weekly predicted incidence to a 1-10 Maryland-relative scale using 95th percentile x 1.2 headroom. Rows preserve source SHA-256 values, seasonality source, benchmark quantile, headroom multiplier, and denominator so alternate scoring branches can coexist. `tickbiterisk risk lookup` consumes this derived file for county/date JSON lookup. Carries `relative_seasonal_baseline`, `static_seasonality_prior`, and `not_weather_adjusted`. |
