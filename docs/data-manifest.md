...
| `noaa_cpc_enso_index` | NOAA CPC Oceanic Nino Index / Relative Oceanic Nino Index | `https://www.cpc.ncep.noaa.gov/data/indices/oni.ascii.txt` and `https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/enso/roni/` | Text/HTML table | Global climate index | 1950-present | ENSO phase climate context candidate | candidate, needs_acquisition, needs_etl, optional | Public federal data; cite NOAA CPC | El Nino / La Nina phase should be tested only as a lagged climate-regime overtone, likely prior winter/spring to model year. Not Maryland-specific and not a causal claim. Preserve ONI/RONI version because NOAA CPC has shifted official monitoring toward RONI while ONI remains a long historical reference. |
| `noaa_psl_mei_v2` | NOAA PSL Multivariate ENSO Index version 2 | `https://psl.noaa.gov/enso/mei/` and `https://psl.noaa.gov/data/timeseries/month/MEIV2/` | HTML/text time series | Global bimonthly climate index | 1979-present for MEI.v2 table | ENSO ocean-atmosphere climate context candidate | candidate, needs_acquisition, needs_etl, optional | Public federal data; cite NOAA PSL | Companion to ONI/RONI if ENSO is tested. MEI.v2 combines multiple ocean-atmosphere variables, so it belongs in a separate lagged feature lane rather than silently replacing local Maryland weather or drought features. |
...
| `model_comparison` | Rolling-origin annual model comparison | `build/etl/model-comparison/model_comparison_runs.csv`, `model_comparison_predictions.csv`, `model_comparison_metrics.csv`, `model_comparison_summary.csv` | CSV | County-year prediction/evaluation | Held-out years 2007-2023 | Model selection benchmark over normalized design matrix | etl_materialized | Derived evaluation artifact; publish with model caveats | Built by `tickbiterisk etl model-compare`; 2026-05-27 run wrote 1 run row, 3,264 prediction rows, 144 metric rows, and 8 summary rows. Summary MAE per 100k after adding the spatial lag lane: `linear_blend_baseline` 18.240783, `prior_year_incidence` 18.32327, `ridge_forecast_safe` 19.094881, `ridge_forecast_spatial` 19.222024, `ridge_forecast_ecology` 21.782782, `ridge_lag_weather_ecology` 22.190121, `empirical_bayes_shrinkage` 22.859609, `trailing_mean_incidence` 22.903909. The spatial and ecology features did not improve held-out MAE, so the selected public branch remains `linear_blend_baseline`. |
...
- NOAA CPC/PSL ENSO indices, citizen-science tick observations, GBIF tick occurrences, and the composite ecological-pressure index are catalogued as candidates only. They are not ETL materialized and are not public-default model inputs.
...
- ENSO ONI/RONI/MEI extraction and lag contract if a broad climate-regime feature is tested.
